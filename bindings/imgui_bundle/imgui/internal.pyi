...
class ItemFlags_(enum.Enum):
    ...
    no_window_hoverable_check = enum.auto()  # (= 1 << 8)  # False     // Disable hoverable check in ItemHoverable()
    ...
    has_selection_user_data = enum.auto()  # (= 1 << 11)  # False     // Set by SetNextItemSelectionUserData()
...
class InputTextFlagsPrivate_(enum.Enum):
    ...
    im_gui_input_text_flags_multiline = enum.auto()  # (= 1 << 26)  # For internal use by InputTextMultiline()
...
class ButtonFlagsPrivate_(enum.Enum):
    ...
    # }
    im_gui_button_flags_pressed_on_default_ = enum.auto()  # (= ButtonFlags_PressedOnClickRelease)
...
class InputTextState:
    ...
    initial_text_a: ImVector_char  # value to revert to when pressing Escape = backup of end-user buffer at the time of focus (in UTF-8, unaltered)
    ...
    # bool                    ReloadUserBuf;    /* original C++ signature */
    reload_user_buf: (
    ...
    reload_selection_start: int  # POSITIONS ARE IN IMWCHAR units *NOT* UTF-8 this is why this is not exposed yet.
    ...
    def get_undo_avail_count(self) -> int:
        """(private API)"""
        pass
    # int         GetRedoAvailCount() const   { return IMSTB_TEXTEDIT_UNDOSTATECOUNT - Stb.undostate.redo_point; }    /* original C++ signature */
    ...
    # void        ReloadUserBufAndSelectAll()     { ReloadUserBuf = true; ReloadSelectionStart = 0; ReloadSelectionEnd = INT_MAX; }    /* original C++ signature */
    def reload_user_buf_and_select_all(self) -> None:
        """(private API)"""
        pass
    # void        ReloadUserBufAndKeepSelection() { ReloadUserBuf = true; ReloadSelectionStart = Stb.select_start; ReloadSelectionEnd = Stb.select_end; }    /* original C++ signature */
    def reload_user_buf_and_keep_selection(self) -> None:
        """(private API)"""
        pass
    # void        ReloadUserBufAndMoveToEnd()     { ReloadUserBuf = true; ReloadSelectionStart = ReloadSelectionEnd = INT_MAX; }    /* original C++ signature */
...
class LastItemData:
    ...
    display_rect: ImRect  # Display rectangle (ONLY VALID IF ImGuiItemStatusFlags_HasDisplayRect is set)
    # ImRect                  ClipRect;    /* original C++ signature */
    clip_rect: (
        ImRect  # Clip rectangle at the time of submitting item (ONLY VALID IF ImGuiItemStatusFlags_HasClipRect is set)
    )
...
class NavTreeNodeData:
    ...
    def __init__(self, id_: ID = ID(), in_flags: ItemFlags = ItemFlags(), nav_rect: ImRect = ImRect()) -> None:
        """Auto-generated default constructor with named params"""
        pass
...
class KeyRoutingData:
    ...
    # ImGuiKeyRoutingData()           { NextEntryIndex = -1; Mods = 0; RoutingCurrScore = RoutingNextScore = 255; RoutingCurr = RoutingNext = ImGuiKeyOwner_None; }    /* original C++ signature */
    def __init__(self) -> None:
        pass
...
class InputFlags_(enum.Enum):
    ...
    # ImGuiInputFlags_CondHovered         = 1 << 8,       /* original C++ signature */
    cond_hovered = enum.auto()  # (= 1 << 8)  # Only set if item is hovered (default to both)
    # ImGuiInputFlags_CondActive          = 1 << 9,       /* original C++ signature */
    cond_active = enum.auto()  # (= 1 << 9)  # Only set if item is active (default to both)
    ...
    # ImGuiInputFlags_LockThisFrame       = 1 << 10,      /* original C++ signature */
    lock_this_frame = (
        enum.auto()
    ...
    # ImGuiInputFlags_LockUntilRelease    = 1 << 11,      /* original C++ signature */
    lock_until_release = (
        enum.auto()
    )  # (= 1 << 11)  # Further accesses to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT accepted for polling). Cleared when the key is released or at end of each frame if key is released.
    ...
    # ImGuiInputFlags_RouteFocused        = 1 << 12,      /* original C++ signature */
    route_focused = (
        enum.auto()
    )  # (= 1 << 12)  # (Default) Honor focus route: Accept inputs if window is in focus stack. Deep-most focused window takes inputs. ActiveId takes inputs over deep-most focused window.
    # ImGuiInputFlags_RouteGlobalLow      = 1 << 13,      /* original C++ signature */
    route_global_low = (
        enum.auto()
    )  # (= 1 << 13)  # Register route globally (lowest priority: unless a focused window or active item registered the route) -> recommended Global priority IF you need a Global priority.
    # ImGuiInputFlags_RouteGlobal         = 1 << 14,      /* original C++ signature */
    route_global = (
        enum.auto()
    )  # (= 1 << 14)  # Register route globally (medium priority: unless an active item registered the route, e.g. CTRL+A registered by InputText will take priority over this).
    # ImGuiInputFlags_RouteGlobalHigh     = 1 << 15,      /* original C++ signature */
    route_global_high = (
        enum.auto()
    )  # (= 1 << 15)  # Register route globally (higher priority: unlikely you need to use that: will interfere with every active items, e.g. CTRL+A registered by InputText will be overriden by this)
    # ImGuiInputFlags_RouteAlways         = 1 << 16,      /* original C++ signature */
    route_always = enum.auto()  # (= 1 << 16)  # Do not register route, poll keys directly.
    # Routing polices: extra options
    # ImGuiInputFlags_RouteUnlessBgFocused= 1 << 17,      /* original C++ signature */
    route_unless_bg_focused = (
        enum.auto()
    )  # (= 1 << 17)  # Global routes will not be applied if underlying background/None is focused (== no Dear ImGui windows are focused). Useful for overlay applications.
    ...
    # ImGuiInputFlags_SupportedByShortcut         = ImGuiInputFlags_RepeatMask_ | ImGuiInputFlags_RouteMask_ | ImGuiInputFlags_RouteAlways | ImGuiInputFlags_RouteUnlessBgFocused,    /* original C++ signature */
    supported_by_shortcut = (
        enum.auto()
    )  # (= InputFlags_RepeatMask_ | InputFlags_RouteMask_ | InputFlags_RouteAlways | InputFlags_RouteUnlessBgFocused)
...
class NavHighlightFlags_(enum.Enum):
    ...
    none = enum.auto()  # (= 0)
    # ImGuiNavHighlightFlags_Compact          = 1 << 1,           /* original C++ signature */
    compact = enum.auto()  # (= 1 << 1)  # Compact highlight, no padding
...
class DebugLogFlags_(enum.Enum):
    ...
    # ImGuiDebugLogFlags_EventInputRouting    = 1 << 7,    /* original C++ signature */
    event_input_routing = enum.auto()  # (= 1 << 7)
    # ImGuiDebugLogFlags_EventDocking         = 1 << 8,    /* original C++ signature */
    event_docking = enum.auto()  # (= 1 << 8)
    # ImGuiDebugLogFlags_EventViewport        = 1 << 9,    /* original C++ signature */
    event_viewport = enum.auto()  # (= 1 << 9)
    ...
    event_mask_ = (
        enum.auto()
    )  # (= DebugLogFlags_EventActiveId  | DebugLogFlags_EventFocus | DebugLogFlags_EventPopup | DebugLogFlags_EventNav | DebugLogFlags_EventClipper | DebugLogFlags_EventSelection | DebugLogFlags_EventIO | DebugLogFlags_EventInputRouting | DebugLogFlags_EventDocking | DebugLogFlags_EventViewport)
...
class Context:
    ...
    active_id_using_all_keyboard_keys: bool  # Active widget will want to read all keyboard keys inputs. (FIXME: This is a shortcut for not taking ownership of 100+ keys but perhaps best to not have the inconsistency)
    # ImGuiKeyChord           DebugBreakInShortcutRouting;    /* original C++ signature */
    debug_break_in_shortcut_routing: KeyChord
    ...
    nav_disable_mouse_hover: (
        bool  # When user starts using gamepad/keyboard, we hide mouse hovering highlight until mouse is touched again.
    )
    ...
    scrollbar_click_delta_to_grab_center: (
        float  # Distance between mouse and center of grab box, normalized in parent space. Use storage?
    )
...
class WindowTempData:
    ...
    tree_jump_to_parent_on_pop_mask: (
        ImU32  # Store a copy of !g.NavIdIsAlive for TreeDepth 0..31.. Could be turned into a ImU64 if necessary.
    )
...
class TabItemFlagsPrivate_(enum.Enum):
    ...
    im_gui_tab_item_flags_section_mask_ = enum.auto()  # (= TabItemFlags_Leading | TabItemFlags_Trailing)
...
def translate_windows_in_viewport(viewport: ViewportP, old_pos: ImVec2, new_pos: ImVec2) -> None:
    pass
...
def nav_move_request_resolve_with_past_tree_node(result: NavItemData, tree_node_data: NavTreeNodeData) -> None:
    pass
...
# inline bool             IsModKey(ImGuiKey key)                                      { return key >= ImGuiKey_LeftCtrl && key <= ImGuiKey_RightSuper; }    /* original C++ signature */
def is_mod_key(key: Key) -> bool:
    """(private API)"""
    pass

# ImGuiKeyChord           FixupKeyChord(ImGuiContext* ctx, ImGuiKeyChord key_chord);    /* original C++ signature */
def fixup_key_chord(ctx: Context, key_chord: KeyChord) -> KeyChord:
    """(private API)"""
    pass
...
def is_key_chord_pressed(key_chord: KeyChord, owner_id: ID, flags: InputFlags = 0) -> bool:
    pass

# IMGUI_API void              SetNextItemShortcut(ImGuiKeyChord key_chord);    /* original C++ signature */
def set_next_item_shortcut(key_chord: KeyChord) -> None:
    pass
...
# IMGUI_API bool              SetShortcutRouting(ImGuiKeyChord key_chord, ImGuiID owner_id, ImGuiInputFlags flags = 0);     /* original C++ signature */
def set_shortcut_routing(key_chord: KeyChord, owner_id: ID, flags: InputFlags = 0) -> bool:
...
# IMGUI_API void          TableAngledHeadersRowEx(float angle, float max_label_width = 0.0f);    /* original C++ signature */
def table_angled_headers_row_ex(angle: float, max_label_width: float = 0.0) -> None:
    pass
...
def render_frame(p_min: ImVec2, p_max: ImVec2, fill_col: ImU32, border: bool = True, rounding: float = 0.0) -> None:
    pass
...
# IMGUI_API void          RenderNavHighlight(const ImRect& bb, ImGuiID id, ImGuiNavHighlightFlags flags = ImGuiNavHighlightFlags_None);     /* original C++ signature */
def render_nav_highlight(bb: ImRect, id_: ID, flags: NavHighlightFlags = NavHighlightFlags_None) -> None:
...
def scrollbar_ex(
    bb: ImRect, id_: ID, axis: Axis, p_scroll_v: ImS64, avail_v: ImS64, contents_v: ImS64, flags: ImDrawFlags
) -> bool:
    pass
